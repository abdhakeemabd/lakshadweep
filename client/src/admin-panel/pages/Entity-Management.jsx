...
function EntityManagement() {
  ...
  const handleDelete = async (category_id, categoryName) => {
    ...
    if (!category_id) {
      showDeleteError('Invalid Category ID');
      return;
    }
              ...
              {loading ? (
                <tr>
                  <td colSpan="3" className="text-center py-20">
                    ...
                    <p className="mt-4 text-gray-500 text-[12px]">Loading categories...</p>
                  </td>
                </tr>
              ) : error ? (
                <tr>
                  <td colSpan="3" className="text-center py-20 text-red-500 text-[12px]">
                    Error: {error}
                    <button onClick={fetchCategories} className="ml-3 text-[#007BFF] underline cursor-pointer">Try Again</button>
                  </td>
                </tr>
              ) : categories.length === 0 ? (
                <tr>
                  <td colSpan="3" className="text-center py-20 text-gray-500 text-[12px]">
                    No categories found.
                  </td>
                </tr>
              ) : (
                categories.map((category, index) => (
                  <tr key={category.id || index} className='border-b border-[#dee2e6] last:border-0'>
                    <td className="px-4 py-2 text-[12px] text-[#383838]">{category.name || category.category_name || 'N/A'}</td>
                    <td className="px-4 py-2 text-[12px] text-[#3d3d3d]">
                      {Array.isArray(category.activities)
                        ? category.activities.map(act => act.name || act).join(', ')
                        : (category.activity_names || 'No activities')}
                    </td>
                    <td className="py-2 text-[12px] text-[#383838]">
                      <div className='flex justify-center gap-3 items-center'>
                        <Link to={`/admin/setting/edit-catagory/${category.id}`} className='cursor-pointer border-none bg-transparent p-0'>
                          <img className='img-fluid' src={EditIcon} alt="Edit" />
                        </Link>
                        <button className='cursor-pointer border-none bg-transparent p-0' type='button' onClick={() => handleDelete(category.id, category.name || category.category_name)}>
