...
function EditLocationModal() {
  ...
  useEffect(() => {
    ...
    const handleGlobalClick = (e) => {
      if (e.target.getAttribute('commandfor') === 'add-locatedit-modal') {
        dialog?.showModal()
      }

      if (
        e.target.getAttribute('command') === 'close' &&
        e.target.getAttribute('commandfor') === 'add-locatedit-modal'
      ) {
        handleCloseModal()
      }
    }
  ...
  return (
    <>
      ...
      <dialog ref={dialogRef} id="edit-location-modal" aria-labelledby="edit-location-modal-title" className="fixed inset-0 z-[100] w-full h-full bg-transparent m-0 p-0 max-w-none max-h-none backdrop:bg-black/50 backdrop:backdrop-blur-sm py-3 md:py-7">
              ...
              <div className="modal-body px-6 py-4 bg-white">
                <div className="grid grid-cols-1 md:grid-cols-12 gap-3 md:gap-5">
                  <div className="col-span-12 mb-3 lg:mb-5">
                    <div className="text-[#3D3D3D] font-poppins font-medium text-[13px] ">Location  <span className="text-">*</span>  </div>
